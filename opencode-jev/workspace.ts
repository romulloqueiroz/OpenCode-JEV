...
async function safeTarget(root, name) {
...
export async function readWorkspace(directory, maxBytes, signal) {
...
export function validateDraft(value, maxBytes) {
...
export function candidateFiles(baseline, draft, maxBytes) {
  const files = new Map(baseline.map(f => [f.path, f.content]))
...
export async function applyDraft(directory, baseline, draft, maxBytes, signal) {
  signal?.throwIfAborted()
  const root = await fs.realpath(directory)
  ...
  async function replace(target, content, previous, mode) {
    ...
    try {
      ...
      signal?.throwIfAborted()
      let live = null
      try { live = await fs.readFile(target, "utf8") } catch (error) { if (error.code !== "ENOENT") throw error }
    ...
    } finally { await fs.unlink(staging).catch(error => { if (error.code !== "ENOENT") throw error }) }
  }
  try {
    ...
    for (const file of written.reverse()) {
      const target = await safeTarget(root, file.path)
      let live = null
      try { live = await fs.readFile(target, "utf8") } catch (readError) { if (readError.code !== "ENOENT") continue }
