...
import { readWorkspace, validateDraft, candidateFiles, applyDraft, hash } from "./workspace.mjs"
import { runBridge } from "./io.mjs"
...
export function decodeDraft(response, maxBytes) {
...
async function saveRound(directory, runID, round, value) {
...
export function createHarness(overrides = {}) {
  const deps = { readWorkspace, applyDraft, runBridge, saveRound, ...overrides }
  return async function runHarness({ client, directory, sessionID, model, variant, task, config, signal, progress = async () => {} }) {
    ...
    let best, feedback = "", stalls = 0, rounds = 0, audit, stopReason = "revision budget"
    const seen = new Set()
    ...
    try {
      for (let round = 0; round <= config.maxRevisions; round++) {
        ...
        const payload = { task, files: candidateFiles(baseline, draft, config.maxSourceBytes), timeout: config.timeout,
          ...(rubric ? { rubric } : {}), ...(best ? { previous_report: best.report } : {}) }
        const result = deps.evaluate ? await deps.evaluate(payload, { signal })
